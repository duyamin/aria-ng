    ...
    def validate(self, value):
        ...
        if self.cls and not isinstance(value, self.cls):
            try:
                ...
                raise InvalidValueError('property must be coercible to %s: %s=%s' %
                                        (self.cls.__name__, self.name, safe_repr(value)))

        return value


def has_validated_props_iter_validated_property_names(self):
...
def has_validated_properties(cls):
    ...
    # Bind methods
    setattr(cls, '_iter_validated_property_names',
            MethodType(has_validated_props_iter_validated_property_names, None, cls))
    setattr(cls, '_iter_validated_properties',
            MethodType(has_validated_properties_iter_validated_properties, None, cls))
...
def validated_property(cls, default=None, required=False, status='supported'):
    ...
    """
    def decorator(func):
        return Prop(fn=func, cls=cls, default=default, required=required, status=status)
    return decorator

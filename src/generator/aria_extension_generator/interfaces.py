...
def has_interfaces_interface(obj, name):
    if not hasattr(obj, 'interfaces'):
        setattr(obj, 'interfaces', {})

    interface = obj.interfaces.get(name)

    if interface is None:
        interface_template = obj.__class__.INTERFACES.get(name)
        if interface_template is None:
            raise AttributeError('no interface: %s' % name)
        interface = Interface()
        for method in interface_template.itervalues():
            setattr(interface, method.name, MethodType(method.fn, obj, None))
        obj.interfaces[name] = interface

    return interface


def has_interfaces_iter_interface_names(obj):
    return obj.__class__.INTERFACES.iterkeys()


def has_interfaces_iter_interface_methods(obj):
    for interface_name, interface in obj.__class__.INTERFACES.iteritems():
...
def has_interfaces(cls):
    ...
    setattr(cls, 'iter_interface_methods', MethodType(has_interfaces_iter_interface_methods, None,
                                                      cls))
...
def interfacemethod(interface_name, name=None):
    def decorator(func):
        return InterfaceMethod(func, interface_name, name)
    return decorator
